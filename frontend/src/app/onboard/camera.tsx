...
const CHECKLIST = [
  ...
  'If a private door is in view, mask it on the computer before you start — the masked pixels never reach the detector.',
...
export default function Camera() {
  const { residentId, residentName, camera, setCamera } = useSession();
  ...
  return (
    <Screen>
      ...
      <Txt kind="body" tone="muted" style={{ marginTop: sp(3) }}>
        One camera, in the room she spends her day in. You will see this room here in
        Settings — {residentName} is never described to you by room.
      </Txt>
      ...
      <Txt kind="caption" tone="muted" style={{ marginTop: sp(3) }}>
        Bedrooms and bathrooms are not on this list, and cannot be added. Dhyaan
        refuses them.
      </Txt>
      ...
      <Field
        ...
        hint="This helps Dhyaan name the spot she is in — her armchair, the table — instead of guessing."
        multiline
        maxLength={300}
      ...
      <View style={{ marginTop: sp(6), gap: sp(2) }}>
        ...
        {test.kind === 'idle' && (
          <Txt kind="caption" tone="muted">
            Start the camera on her computer, then tap Test. You will never see a
            picture here — only whether it is running.
          </Txt>
        )}
        {test.kind === 'testing' && (
        ...
        {test.kind === 'unreachable' && (
          <View style={{ gap: sp(1) }}>
            ...
            <Txt kind="caption" tone="muted">
              Start the camera on her computer, then tap Test. You can finish setup
              without it and test later.
            </Txt>
          </View>
        )}
      </View>
