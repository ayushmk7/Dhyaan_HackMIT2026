...
// The setup overview and the data-source readout used to sit above and below
// the buttons; both are gone. The disclaimer stays: it is a safety claim.
//
...
import { Btn, Entrance, Mark, Screen, Txt } from '@/components';
  ...
  return (
    <Screen scroll={false} wash style={{ justifyContent: 'space-between' }}>
      ...
      <Entrance index={2} style={{ gap: sp(3) }}>
        ...
        >
          <View pointerEvents="none" style={{ alignSelf: 'stretch' }}>
            <Btn kind="quiet" label={copy.exploreDemo} onPress={familyDemo} style={{ alignSelf: 'stretch' }} />
          </View>
        </Pressable>
        <Txt kind="caption" tone="muted" style={{ textAlign: 'center', marginTop: sp(3) }}>{/* voice-ok */}
          {copy.disclaimer}
        </Txt>
      </Entrance>
    </Screen>
