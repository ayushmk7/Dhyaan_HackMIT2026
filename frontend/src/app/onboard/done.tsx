...
const EXPECT = [
  'For the first few days Dhyaan mostly repeats what you told it. That is the point — it says what it was told, and what it saw, separately.',
      ...
      <Entrance index={1}>
        ...
        <Card style={{ marginTop: sp(3) }}>
          ...
          <Txt kind="caption" tone="muted" style={{ marginTop: sp(2) }}>
            {facts.length === 0
              ? 'Nothing yet — you skipped every question. You can add notes any time in Settings, and Dhyaan will say it wasn’t told until you do.'
        ...
        {!!error && (
          <View style={{ marginTop: sp(3), gap: sp(2) }}>
            ...
            <Txt kind="caption" tone="muted">
              Nothing has been saved to her home hub. Your answers are still on this
              phone — try again, or skip in and finish from Settings.
            </Txt>
            <Btn
              kind="quiet"
              label="Skip in without saving"
              onPress={() => { session.finishOnboarding(); router.replace('/(family)'); }}
            />
          </View>
        )}
      </Entrance>
