    ...
    if (/ok|okay|alright|fine|worried/.test(q)) {
      ...
      return reply(
        p.sentence
          ...
          : 'Nothing has happened yet today. The camera is on, nothing noticed, no alerts.',
        [itemCite('meal_observed')],
      );
    }
    return reply(
      'I can answer from three things: what you told us about her, what the camera noticed, and the pattern it has learned. Nothing I have answers that yet. Try her meals, her afternoons, her nights, or whether anyone visited.',
      [],
      true,
      'no_data',
    );
